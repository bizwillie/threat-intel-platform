...
class User:
    ...
    def __init__(self, username: str, email: str, roles: List[str], user_id: str):
        ...
        self.id = user_id  # Alias for compatibility with layer generation
...
async def get_current_user(
    ...
    # Also check utip-frontend client roles
    frontend_roles = resource_access.get("utip-frontend", {}).get("roles", [])
    roles.extend(frontend_roles)
...
async def get_current_user_optional(
    ...
    """
    Optional authentication - returns None if no token provided or token is invalid.
